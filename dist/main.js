...
"use strict";
eval("\n\nvar RM = 0.4;\nvar RN = 0.6;\n\nfunction getImageNormal(gazePoints) {\n  var leftEye = gazePoints.leftEye,\n      rightEye = gazePoints.rightEye,\n      noseTip = gazePoints.noseTip,\n      mouthLeft = gazePoints.mouthLeft,\n      mouthRight = gazePoints.mouthRight;\n\n\n  var noseBase = [(leftEye[0] + rightEye[0]) / 2, (leftEye[1] + rightEye[1]) / 2];\n  var mouth = [(mouthLeft[0] + mouthRight[0]) / 2, (mouthLeft[1] + mouthRight[1]) / 2];\n\n  var n = [mouth[0] + (noseBase[0] - mouth[0]) * RM, mouth[1] + (noseBase[1] - mouth[1]) * RM];\n\n  var theta = Math.acos(((noseBase[0] - n[0]) * (noseTip[0] - n[0]) + (noseBase[1] - n[1]) * (noseTip[1] - n[1])) / Math.hypot(noseTip[0] - n[0], noseTip[1] - n[1]) / Math.hypot(noseBase[0] - n[0], noseBase[1] - n[1]));\n  var tau = Math.atan2(n[1] - noseTip[1], n[0] - noseTip[0]);\n\n  var m1 = ((noseTip[0] - n[0]) * (noseTip[0] - n[0]) + (noseTip[1] - n[1]) * (noseTip[1] - n[1])) / ((noseBase[0] - mouth[0]) * (noseBase[0] - mouth[0]) + (noseBase[1] - mouth[1]) * (noseBase[1] - mouth[1]));\n  var m2 = Math.cos(theta) * Math.cos(theta);\n  var a = RN * RN * (1 - m2);\n  var b = m1 - RN * RN + 2 * m2 * RN * RN;\n  var c = -m2 * RN * RN;\n\n  var delta = Math.acos(Math.sqrt((Math.sqrt(b * b - 4 * a * c) - b) / (2 * a)));\n\n  var fn = [];\n  var sfn = [];\n  fn.push(Math.sin(delta) * Math.cos(tau));\n  fn.push(Math.sin(delta) * Math.sin(tau));\n  fn.push(-Math.cos(delta));\n\n  var alpha = Math.PI / 12;\n  sfn.push(0);\n  sfn.push(Math.sin(alpha));\n  sfn.push(-Math.cos(alpha));\n\n  // PITCH:X YAW:Y ROLL:X\n\n  /*\n    live2d rotation order: ZXY\n    live2d coordinate           my coordinate           my coordinate\n    angle Z                     z axis                  Yaw\n    angle X                     y axis                  Pitch\n    angle Y                     x axis                  Roll\n    my coordinate is same as the paper:\n    Estimating Gaze from a Single View of a Face\n    link: ieeexplore.ieee.org/document/576433/\n     */\n\n  // (w, x, y, z) is Euler quaternion\n  //\n  var angle = Math.acos((sfn[0] * fn[0] + sfn[1] * fn[1] + sfn[2] * fn[2]) / Math.sqrt(sfn[0] * sfn[0] + sfn[1] * sfn[1] + sfn[2] * sfn[2]) / Math.sqrt(fn[0] * fn[0] + fn[1] * fn[1] + fn[2] * fn[2]));\n  var w = Math.cos(0.5 * angle);\n  var x = sfn[1] * fn[2] - sfn[2] * fn[1];\n  var y = sfn[2] * fn[0] - sfn[0] * fn[2];\n  var z = sfn[0] * fn[1] - sfn[1] * fn[0];\n\n  var l = Math.sqrt(x * x + y * y + z * z);\n  x = Math.sin(0.5 * angle) * x / l;\n  y = Math.sin(0.5 * angle) * y / l;\n  z = Math.sin(0.5 * angle) * z / l;\n\n  var yaw = void 0;\n  var pitch = void 0;\n  var roll = void 0;\n  roll = Math.atan2(2 * (w * x + y * z), 1 - 2 * (x * x + y * y));\n  pitch = Math.asin(2 * (w * y - z * x));\n  yaw = Math.atan2(2 * (w * z + x * y), 1 - 2 * (y * y + z * z));\n\n  if (sfn[0] < 0.1 && sfn[1] < 0.1) {\n    roll = 1.5 * Math.atan2(rightEye[1] - leftEye[1], rightEye[0] - leftEye[0]);\n  }\n\n  yaw = Math.max(-30, Math.min(30, yaw * 180 / Math.PI));\n  pitch = Math.max(-30, Math.min(30, pitch * 180 / Math.PI));\n  roll = Math.max(-30, Math.min(30, roll * 180 / Math.PI));\n\n  // console.log('Yaw: ' + yaw + ' Pitch: ' + pitch + ' Roll: ' + roll)\n\n  return {\n    yaw: yaw,\n    pitch: pitch,\n    roll: roll\n  };\n}\n\nmodule.exports = {\n  getImageNormal: getImageNormal\n};\n\n//# sourceURL=webpack:///./src/resolve_landmarks.js?");
